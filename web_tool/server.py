...
from Session import manage_session_folders, SESSION_FOLDER
from SessionHandler import SessionHandler
from Utils import get_random_string, class_prediction_to_img
...
def setup_sessions():
    ...
    bottle.request.client_ip = bottle.request.environ.get("HTTP_X_FORWARDED_FOR") or bottle.request.environ.get(
        "REMOTE_ADDR")


def manage_sessions():
    """This method is called before every request. Checks to see if there is a session associated
    with the current request.
    If there is then update the last interaction time on that session.
    """

    if SESSION_HANDLER.is_expired(
            bottle.request.session.id):  # Someone is trying to use a session that we have deleted due to inactivity
        ...
        SESSION_HANDLER.touch_session(
            bottle.request.session.id)  # let the SESSION_HANDLER know that this session has activity
...
def record_correction():
    ...
    SESSION_HANDLER.get_session(bottle.request.session.id).model.add_sample(tdst_row, bdst_row, tdst_col, bdst_col,
                                                                            class_idx)
...
def pred_patch():
    ...
    naip_data, naip_crs, naip_transform, naip_bounds, naip_index = DATASETS[dataset][
        "data_loader"].get_data_from_extent(extent)
    naip_data = np.rollaxis(naip_data, 0,
                            3)  # we do this here instead of get_data_by_extent because not all GeoDataTypes will have a channel dimension
    ...
    assert (output.shape[2] < output.shape[0] and output.shape[2] < output.shape[
        1]), "The model function should return an image shaped as (height, width, num_classes)"  # assume that num channels is less than img dimensions
...
def pred_tile():
    ...
    try:
        naip_data, raster_profile, raster_transform, raster_bounds, raster_crs = DATASETS[dataset][
            "data_loader"].get_data_from_shape(geom["geometry"])
...
def main():
    ...
    parser.add_argument("--storage_path", action="store", dest="storage_path", type=str,
                        help="Path to directory where output will be stored", default=None)
    ...
    subparsers = parser.add_subparsers(dest="subcommand",
                                       help="Help for subcommands")  # TODO: If we use Python3.7 we can use the required keyword here
    ...
    parser.add_argument("--remote_host", action="store", dest="remote_host", type=str, help="RabbitMQ host",
                        default="0.0.0.0")
    parser.add_argument("--remote_port", action="store", dest="remote_port", type=int, help="RabbitMQ port",
                        default=8080)
    ...
    app.add_hook("before_request",
                 manage_sessions)  # before every request we want to check to make sure there are no session issues

    # API paths
    app.route("/predPatch", method="OPTIONS",
              callback=do_options)  # TODO: all of our web requests from index.html fire an OPTIONS call because of https://stackoverflow.com/questions/1256593/why-am-i-getting-an-options-request-instead-of-a-get-request, we should fix this
